...
async function start() {
  ...
  if (fs.existsSync("./lnd/data/chain/bitcoin/testnet/wallet.db")) {
...
async function unlockExistingWallet() {
  ...
  const lnd = lnService.lightningDaemon({
    ...
    macaroon: Buffer.from(
      fs.readFileSync("./lnd/data/chain/bitcoin/testnet/admin.macaroon"),
      "base64"
    ).toString("hex"),
...
async function createNewWallet() {
  ...
  fs.writeFileSync(
    "./lnd/secret.json",
    JSON.stringify({ seed, password, connect }, null, 2)
  );
...
async function lndconnect() {
  ...
  const macaroonPath = "./lnd/data/chain/bitcoin/testnet/admin.macaroon";
